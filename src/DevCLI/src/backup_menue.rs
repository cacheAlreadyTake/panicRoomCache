    ...
    pub fn create(){
        ...
        fs::rename(format!("{}PatnicRoom-Developer-Edition",root),format!("{}Backup",root));
    }
    ...
    pub fn load(){
        ...
        let root    = helpers::filepaths::get_root();
        let back_up = format!("{}Backup",root);
        ...
        Command::new("cp").arg("-r").arg("../../Backup/*").arg("../../PatnicRoom-Developer-Edition").status();
    ...
    fn print_folder_copy(workspace:&[&str]){
        for current_workspace in workspace.iter(){
            ...
            helpers::text_formater::print_white(" into Backup\n");
        }
    }
    ...
    fn print_folder_load(workspace:&[&str]){
        for current_workspace in workspace.iter(){
            ...
            helpers::text_formater::print_cyan(current_workspace);
            helpers::text_formater::print_white(" of ./Backup\n");
        }
    }
    ...
    fn print_folder_reset(workspace:&[&str]){
        for current_workspace in workspace.iter(){
            ...
            helpers::text_formater::print_cyan(current_workspace);
            helpers::text_formater::print_white(" from ./Backup\n");
        }
    }