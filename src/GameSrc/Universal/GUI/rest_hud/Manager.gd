...
func _ready() -> void:
	...
	#get_node("workshop").initialice(basepath+"_005_Common_Station/Station.tscn",ENUM.INPUT_REST.STATION.WORKSHOP)
	get_node("workshop").initialice(basepath+"_005_Common_Station/Station.tscn",ENUM.INPUT_REST.STATION.WORKSHOP)
	get_node("terminal").initialice(basepath+"_005_Common_Station/Station.tscn",ENUM.INPUT_REST.STATION.TERMINAL)
	get_node("retreat").initialice(basepath+"_005_Common_Station/Station.tscn",ENUM.INPUT_REST.STATION.RETREAT)
