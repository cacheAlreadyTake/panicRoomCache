...
func fillUpCategory():
	...
	for cat in ENUM.KITS.CATEGORY.keys():  
		var icon = load("res://Media/GUI/KitSymbols/"+cat.to_lower()+".png")
...
func setKitparts(catEnumNr):
	...
	var kitpartEnt = API_001_Atlas.getKitGroup(catEnumNr-1)
	baseBox.loadedKitPartEntites = kitpartEnt.values()
	var nameToShow = kitpartEnt.keys()
	
	for kitpart in nameToShow: baseBox.add_item(API_001_Atlas.getKitByDrirectName(kitpart).name()) 
	baseBox.addEscape()
...
func setSideBoxes():
	...
	for entry in kitpart.modArr.keys():   modBox.add_item(entry)
