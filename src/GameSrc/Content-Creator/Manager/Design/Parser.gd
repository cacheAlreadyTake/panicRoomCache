...
static func parseRowData(root):
	...
	var form       = Util.JSONParser.fileToDictionary("res://GameData/Alecandria/Forms/"+fileOfForm)
	var allInputs  = root.compInput
	
	var ent = Entity.new()
	
	for row in allInputs.values():   
		ent.addComponent(_Comp.getComponents()[row.compToString].new(row.value))
	...
	for i in form["mirrored values"].size() :
		...
		ent.addComponent(_Comp.getComponents()[paste].new(ent.getCompValue(copy)))
	
	for i in form["static values"].size() :
		var comp  = form["mirrored values"].keys()[i]
		var value = form["mirrored values"].values()[i]
		ent.addComponent(_Comp.getComponents()[comp].new(ent.getCompValue(value)))
