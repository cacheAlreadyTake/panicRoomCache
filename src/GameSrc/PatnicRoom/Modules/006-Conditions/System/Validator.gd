...
static func validate(toValidate:Array,validations:Array) -> bool :
	...
	for i in toValidate.size():
		...
		if rule is Array and rule[0]=="COMPARE":   
			if !checkOptions(rule[1],parameter): 
				isValid = false
		elif !isValid(parameter,rule): 
			isValid = false
...
static func isComp(comp:String) -> bool :   return _Comp.getComponents().keys().has(comp)
