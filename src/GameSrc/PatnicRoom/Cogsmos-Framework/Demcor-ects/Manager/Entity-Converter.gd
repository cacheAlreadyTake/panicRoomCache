...
func convertEntity(toParse:Entity) -> Entity:
	var metaData = getEntityType(toParse)
	var ent      = DemocrECTS_EntityManager.convertEntity(metaData,toParse)
	
	_addEntityToAtlas(ent, metaData["flag"])
	return ent as Entity
...
func getEntityType(toParse) -> Dictionary:
	...
	DemokrECTS.failed_parsers[toParse] = toParse
	return {}
...
func _addEntityToAtlas(ent:Entity,keyword):
	match keyword:
		...
		"F_7_ACTOR_ATTRIBUTES_SPECIALTY" : API_001_Atlas.Specialties().addEntity(ent)
