...
var ARISPROCGENES := _Enum_ArisProcgenes.new()
...
var BEHAVIOUR    := _Enum_Behaviour.new()
...
func _ready() -> void:
	allSubEnums  = [DIOJSONES,SOKRATILES,ENTROPY,ITEMS,KITS,PLAYER,INPUT_PANIC,INPUT_REST,MINIMAL_COMP,LEC,FILE_PATHS,ARISPROCGENES,BEHAVIOUR]
	var cache = []
