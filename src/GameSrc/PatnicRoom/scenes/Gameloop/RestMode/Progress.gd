...
func _ready() -> void:
	_005_Common_StationMeta["MEAT"]["bar"].initialice(API_005_Player.getStations().meat,meatRtl)
	_005_Common_StationMeta["MAGIC"]["bar"].initialice(API_005_Player.getStations().magic,magicRtl)
	_005_Common_StationMeta["MATRIX"]["bar"].initialice(API_005_Player.getStations().matrix,matrixRtl)
